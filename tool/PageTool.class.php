...
class pageTool{
	...
	public function show(){
		...
		$parse = parse_url($uri); //将地址栏信息拆成根目录和后面字符串的数组


		$param = array();
		if(isset($parse['query'])){
			parse_str($parse['query'],$param); //将地址栏后面的字符串拆成数组
		}
        ...
        $url = $parse['path'] . '?';
        if(!empty($param)) {
            $param = http_build_query($param);//将数组拆成类似于id=1&cat=2的字符串
            $url = $url . $param . '&';
        }
